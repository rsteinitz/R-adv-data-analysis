...
## 9. Nonparametric Alternatives

Sometimes, the assumptions of a t-test or ANOVA are not reasonable.

For example:

- the response variable is strongly skewed
- there are extreme outliers
- the data do not look approximately normal

In those cases, we may use nonparametric alternatives.

### 9.1 Wilcoxon Test

The Wilcoxon test is a common alternative to a two-sample t-test.

```{r}
wilcox.test(cortisol_ng_ml ~ injury_label, data = data)
```

### 9.2 Kruskal-Wallis Test

The Kruskal-Wallis test is a common alternative to ANOVA for comparing three or more groups.

```{r}
kruskal.test(cortisol_ng_ml ~ age_class, data = data)
```

### Important note

Nonparametric tests are not automatically "better." They answer a slightly different question and are usually used when the assumptions of a standard test are not reasonable.

For this course, the most important thing is to understand the decision process.

## 10. Summary: Choosing the Right Group Comparison
...
| Question structure | Common test |
|---|---|
...
| Two groups, non-normal data | Wilcoxon test |
| Three or more groups, non-normal data | Kruskal-Wallis test |

# Skills Application -- Lab

## 11. Lab Challenge: Compare Groups in Your Own Dataset
...
1. Open your ongoing R Markdown file: `Lastname_Firstname_Data.Rmd`
2. Load your dataset and continue working in your script.
3. Choose one quantitative response variable.
4. Choose one grouping variable.
5. Count how many groups are in your grouping variable.
6. Create at least one plot comparing the response variable across groups.
   - boxplot
   - violin plot
   - jitter plot
7. Decide which test is most appropriate.
8. Run the test if appropriate.
9. Write 2-3 sentences interpreting the result biologically.

## 12. Template for Your Answer
...
```{r}
# Run your test here, if appropriate
```
