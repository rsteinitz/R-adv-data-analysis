...
Here, we only have one sample of cortisol values, and we are going to compare the mean to a known value.
...
# mean <- mean(data$cortisol_ng_ml, na.rm = T)

# ggplot(data, aes(x = cortisol_ng_ml)) +
#   geom_density() +
#   geom_vline(xintercept = 20, color = "red", linetype = "dashed") +
#   geom_vline(xintercept = mean, color = "grey40", linetype = "solid") 
...
# x-axis labels aren't great... 

# Make injury labels easier to read
data <- data %>%
  mutate(
    injury_label = if_else(injury_status == 1, "Injured", "Not injured")
  ) # create a new variable; use if_else() to look for injury status (a numeric variable); assign labels (characters, words)
...
# true statistical means + CIs
ggplot(data, aes(injury_label, cortisol_ng_ml)) +
  ...
  theme_bw() +
  labs(x = "Injury", y = "Cortisol (ng/mL)")
...
### 6.3 Plot
Transforming your data to the long format (where every column is a variable and every row is an observation) is essential for plotting in R. This structure aligns with tidy data principles, unlocking the ability to easily group, color, and facet graphs using ggplot and other packages. 
```{r}
